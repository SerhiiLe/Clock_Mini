...
А вообще по поиску можно найти много вариантов, от таких сложных и функциональных, как я привёл, так и очень простые, но вполне рабочие.
...
а создание как "железа", так и прошивки позволило мне отвлечься от рутины будней. И не дать заплесневеть мозгам.
...
9. У чипа DS1307 есть проблема 2038 года, из-за использования 32 бит регистров для хранения времени. По этому я не храню время в обычном формате секунд с 1970 года,
вместо этого я считаю секунды с 2020 года, а затем делаю пересчёт в системные 64 бит. Это превращает проблему 2038 года в проблему 2088 года. Минус в том, что все
встроенные в библиотеку методы перестают работать, с другой стороны мне нужны только секунды.

10. При написании старого проекта у меня был вопрос, как сделать из статичной странички динамическую и приемлемо выглядящую как на большом мониторе, так и на телефоне.
Динамика создаётся javascript и JSON. А оформление я сделал на основе таблиц. Как показала практика у таблицы это относительно быстро и просто, есть ограничения.
