...
const byte fontNormal2[][5] PROGMEM = {
	...
	{0x00, 0x00, 0x00, 0x00, 0x00} // ' ' 32 11
};
...
int16_t drawMedium(const char c, int16_t x) {
	...
	if(c >= 1 && c <= 8) {
		...
		switch (gs.tiny_clock) {
			...
			case FONT_WIDE:
			case FONT_WIDE2:
				font = gs.tiny_clock == FONT_WIDE ? (byte*)fontBold: (byte*)fontWide;
				fontWidth = 6;
				cw = 6;
				if(c == ' ') {
					cn = 11; cw = 4;
...
int16_t printMedium(const char* txt, int16_t pos, uint8_t limit) {
	int16_t i = 0;
	if( txt[0]==' ' ) pos += gs.tiny_clock==FONT_WIDE || gs.tiny_clock==FONT_WIDE2 ? -1:
		gs.tiny_clock==FONT_NORMAL2 ? -2: 1;
